...
def seo_audit(url, level, scope, competitor_url):
    ...
    if os.path.exists(crawl_data_path) and os.stat(crawl_data_path).st_size > 0:
     ...
     113|     final_report_data = calculate_seo_score(all_page_results, domain_checks)
114|     final_report_data["audit_duration_s"] = round(time.time() - start_time, 2)
115|     os.makedirs("reports", exist_ok=True)
